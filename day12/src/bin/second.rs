...
fn main() {
    ...
    let lines: Vec<(String, Vec<u64>)> = Vec::from_iter(
        file_reader
            .lines()
            .filter_map(|line| line.ok())
            .map(|line| {
                let split = line.split(" ").collect::<Vec<&str>>();
                let mut springs = split.first().unwrap().to_string();
                springs = (springs.clone() + "?").repeat(4) + &springs + ".";
                let groups_str = split
                    .last()
                    .unwrap()
                    .to_string();
                let groups = ((groups_str.clone() + ",").repeat(4) + &groups_str)
                    .split(",")
    ...
    // let mut possible_placements: Vec<Vec<Vec<u64>>> = Vec::new();
    // let line = ("???#????#???.", [2, 4]);
    let mut result = 0;
    for line in lines.iter() {
        // println!("line: {:?}", line);
        let mut possible_placements_for_spring: Vec<Vec<u64>> = Vec::new();
        let mut first = true;
        for group_len in line.1.iter() {
            // println!("group_len: {}", group_len);
            let mut non_dots: u64 = line.0[0..*group_len as usize]
                .chars()
                .filter(|c| c != &'.')
                .count() as u64;
            // println!("non_dots 0: {}", non_dots);
            let mut possible_springs_for_group: Vec<u64> = Vec::new();
            ...
            for i in 1 as usize..line.0.len() - *group_len as usize {
                    ...
                    && line.0.chars().nth(i - 1).unwrap() != '#'
                {
                    ...
                    if first {
                        ...
                        possible_springs_for_group.push(i as u64);
                        continue;
                    }
                    possible_springs_for_group.push(i as u64);
                }
        ...
        //     possible_placements_for_spring
        // );
        let mut count_placements: Vec<Vec<u64>> = Vec::new();
        ...
        for i in 1..possible_placements_for_spring.len() {
            ...
            let mut count_placements_for_group: Vec<u64> = Vec::new();
            ...
            for j in 0..this_len {
                ...
                let count = possible_placements_for_spring[i - 1]
                    ...
                    .sum::<u64>();
                // println!("count: {}", count);
                count_placements_for_group.push(count as u64);
            }
            count_placements.push(count_placements_for_group);
            prev_len = next_len;
        }
        // println!("count_placements: {:?}", count_placements);
        let count = count_placements.last().unwrap().iter().sum::<u64>();
        result += count;
        // println!("count: {}", count);
        // Get std input
