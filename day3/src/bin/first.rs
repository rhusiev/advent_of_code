...
fn in_occupied(index: (usize, usize), occupied_indexes: &Vec<(usize, usize, usize)>) -> bool {
    occupied_indexes
        .iter()
        .any(|occupied_index| index.0 == occupied_index.0 && index.1 >= occupied_index.1 && index.1 <= occupied_index.2)
}
...
fn get_adjacent_numbers(index: (usize, usize), engine: &Vec<String>, occupied_indexes: &mut Vec<(usize, usize, usize)>) -> Vec<String> {
    let (width, height) = (engine[0].len(), engine.len());
    let mut adjacent_numbers: Vec<String> = Vec::new();
    match index {
        (0, 0) => {
            push_possible_digit((0, 1), &mut adjacent_numbers, &engine[0], occupied_indexes);
            push_possible_digit((1, 0), &mut adjacent_numbers, &engine[1], occupied_indexes);
            push_possible_digit((1, 1), &mut adjacent_numbers, &engine[1], occupied_indexes);
        }
        (y, x) if y == height - 1 && x == width - 1 => {
            push_possible_digit((y - 1, x - 1), &mut adjacent_numbers, &engine[y - 1], occupied_indexes);
            push_possible_digit((y - 1, x), &mut adjacent_numbers, &engine[y - 1], occupied_indexes);
            push_possible_digit((y, x - 1), &mut adjacent_numbers, &engine[y], occupied_indexes);
        }
...
fn main() {
    ...
    let numbers: Vec<u32> = symbol_indexes
        .iter()
        .map(|index| get_adjacent_numbers(*index, &engine, &mut occupied_indexes))
        .flatten()
